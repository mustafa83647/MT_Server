...
Version: 10.0.1 (Titanium Build - Stable)
Lines of Code: 2200+
    ...
    def check_ip(self, ip: str) -> tuple[bool, str]:
        ...
        if ip in self.failed_logins:
            ...
            if current_time < lockout_time:
                return False, f"تم حظر IP مؤقتاً. حاول بعد {int((lockout_time - current_time) / 60)} دقيقة."
...
class LoggerManager:
    ...
    def log(self, source: str, message: str, is_safe: bool = False, color: str = "#64748b"):
        with self.lock:
            ...
            self.logs.append(f"<span style='color:#64748b;'>[{timestamp}]</span> <b style='color:{source_color};'>[{source}]</b> {clean_msg}")
...
class PlayitDaemon:
    ...
    def _run(self):
        ...
        if not secret:
            ...
            self.logger.log("Playit", "❌ خطأ: لم يتم العثور على PLAYIT_SECRET!", is_safe=True)
            return
        env = os.environ.copy()
        env["HOME"] = DATA_DIR
        self.logger.log("Playit", "🔄 جاري بدء الاتصال بخوادم Playit...", is_safe=True)
        try:
            ...
            for line in self.process.stdout:
                ...
                if "error" in clean_line.lower() or "fail" in clean_line.lower():
                    self.logger.log("Playit", f"❌ {html.escape(clean_line)}", is_safe=True)
                elif "tunnel" in clean_line.lower() or "connected" in clean_line.lower():
                    self.logger.log("Playit", f"🌐 {html.escape(clean_line)}", is_safe=True)
        except Exception as e:
            self.logger.log("Playit", f"❌ انهيار في الشبكة: {e}", is_safe=True)
            self.status = "error"
class BackupManager:
    def __init__(self, logger: LoggerManager):
        self.logger = logger
    ...
    def _create_backup(self):
        ...
        try:
            self.logger.log("Backup", "⏳ جاري إنشاء نسخة احتياطية...", is_safe=True)
            os.makedirs(self.backup_dir, exist_ok=True)
            timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
            shutil.make_archive(os.path.join(self.backup_dir, f"world_backup_{timestamp}"), 'zip', self.world_dir)
            self.logger.log("Backup", f"✅ اكتملت النسخة الاحتياطية بنجاح.", is_safe=True)
        except Exception as e:
            self.logger.log("Backup", f"❌ فشل النسخ: {e}", is_safe=True)
        finally:
            self.is_backing_up = False
...
class MinecraftDaemon:
    ...
    def setup_environment(self) -> bool:
        self.logger.log("النظام", "🛠️ جاري فحص بيئة السيرفر...", is_safe=True)
        ...
        props_path = os.path.join(DATA_DIR, 'server.properties')
        if not os.path.exists(props_path):
            with open(props_path, 'w') as f: f.write("online-mode=false\nserver-port=25565\n")
        fabric_jar = os.path.join(DATA_DIR, "fabric-server-launch.jar")
        libraries_dir = os.path.join(DATA_DIR, "libraries")
        # التحقق الجذري من وجود ملفات التشغيل والمكتبات
        if not os.path.exists(fabric_jar) or not os.path.exists(libraries_dir):
            self.logger.log("النظام", "⚠️ ملفات السيرفر غير مكتملة. جاري التثبيت الجذري (قد يستغرق دقيقتين)...", is_safe=True)

            # تنظيف أي ملفات تالفة سابقة
            for item in ["fabric-server-launch.jar", "server.jar", "libraries", ".fabric"]:
                path = os.path.join(DATA_DIR, item)
                if os.path.exists(path):
                    if os.path.isdir(path): shutil.rmtree(path)
                    else: os.remove(path)
            installer_path = os.path.join(DATA_DIR, "fabric-installer.jar")
            self.logger.log("النظام", "⬇️ جاري تحميل مثبت Fabric...", is_safe=True)
            subprocess.run(["wget", "-q", "-O", installer_path, "https://maven.fabricmc.net/net/fabricmc/fabric-installer/1.0.1/fabric-installer-1.0.1.jar"], cwd=DATA_DIR)
            self.logger.log("النظام", "⚙️ جاري بناء ملفات السيرفر وتحميل Minecraft الأصلية...", is_safe=True)
            # تشغيل المثبت وانتظاره حتى يكتمل
            install_process = subprocess.run(
                ["java", "-jar", "fabric-installer.jar", "server", "-mcversion", "1.20.4", "-loader", "0.15.7", "-downloadMinecraft"],
                cwd=DATA_DIR, capture_output=True, text=True
            )
            if install_process.returncode == 0 and os.path.exists(fabric_jar):
                self.logger.log("النظام", "✅ تم تثبيت السيرفر بنجاح!", is_safe=True)
                if os.path.exists(installer_path): os.remove(installer_path)
                return True
            else:
                self.logger.log("النظام", f"❌ فشل التثبيت! الخطأ: {install_process.stderr}", is_safe=True)
                return False
        return True
    def start_async(self):
    ...
    def _run(self):
        if not self.setup_environment():
            self.logger.log("Minecraft", "🛑 تم إيقاف التشغيل بسبب فشل في تهيئة الملفات.", is_safe=True)
            return
        ...
        self.logger.log("Minecraft", "🚀 جاري إطلاق السيرفر...", is_safe=True)
        try:
            self.process = subprocess.Popen(java_args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, text=True, bufsize=1, cwd=DATA_DIR)
            for line in self.process.stdout:
                clean_line = ANSI_ESCAPE.sub('', line.strip())
                if not clean_line: continue
                self.logger.log("Minecraft", html.escape(clean_line), is_safe=True)
            ...
            self.process.wait()
            if not self.intentional_stop: self.logger.log("Minecraft", "⚠️ السيرفر توقف بشكل غير متوقع (Crash)!", is_safe=True)
            else: self.logger.log("Minecraft", "🛑 توقف السيرفر بأمان.", is_safe=True)
        except Exception as e:
            self.logger.log("Minecraft", f"❌ فشل في تشغيل الجافا: {e}", is_safe=True)
        finally:
            self.online_players.clear()
            self.process = None
...
def send_command():
    ...
    if cmd.strip() == "!resetworld":
        ...
        logger_mgr.log("النظام", "💥 تم فرمتة العالم القديم بنجاح! أوقف السيرفر وشغله من جديد.", is_safe=True)
        return "OK"
...
def handle_mods():
    ...
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            try: os.remove(os.path.join(mods_path, secure_filename(request.form.get('delete'))))
            except: pass
        return "OK"
...
def download_backup(filename):
    return send_from_directory(backup_mgr.backup_dir, secure_filename(filename), as_attachment=True)
...
def file_manager():
    if request.method == 'POST':
        ...
        try: target_path = security_mgr.sanitize_path(target, DATA_DIR)
        except PermissionError: return "Access Denied", 403
        if action == 'delete':
            try:
    ...
    for root, dirs, files in os.walk(DATA_DIR):
        ...
        for file in files:
            if file.endswith(('.json', '.properties', '.txt', '.log')):
                file_list.append(os.path.relpath(os.path.join(root, file), DATA_DIR))
...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-header"><h2>🛡️ Enterprise</h2></div>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card"><div class="stat-title">حالة السيرفر</div><div class="stat-value" id="status-text">جاري التحميل...</div></div>
            <div class="stat-card"><div class="stat-title">استهلاك المعالج (CPU)</div><div class="stat-value" id="cpu-text">0%</div><div class="chart-container"><canvas id="cpuChart"></canvas></div></div>
            <div class="stat-card"><div class="stat-title">استهلاك الرام (RAM)</div><div class="stat-value" id="ram-text">0%</div><div class="chart-container"><canvas id="ramChart"></canvas></div></div>
            <div class="stat-card"><div class="stat-title">اللاعبين المتصلين</div><div class="stat-value" id="players-count">0</div></div>
        </div>
        <div id="console" class="tab-content active">
            ...
            <div class="console-wrapper">
                ...
                <div class="console-input-area">
                    <input type="text" id="cmd" class="console-input" placeholder="اكتب أمر السيرفر هنا..." onkeypress="if(event.key === 'Enter') sendCmd()">
                    <button class="console-btn" onclick="sendCmd()">إرسال</button>
                </div>
            </div>
        </div>
        <div id="players" class="tab-content"><h3 style="margin-top:0; color:var(--primary);">👥 إدارة اللاعبين المتصلين</h3><div id="players-list"><p style="color: var(--text-muted);">جاري التحميل...</p></div></div>
        ...
        <div id="backups" class="tab-content">
            <h3 style="margin-top:0; color:var(--primary);">💾 نظام النسخ الاحتياطي</h3>
            <button class="btn btn-blue" onclick="createBackup()" style="width: 100%; justify-content: center; padding: 15px; font-size: 16px; margin-bottom: 20px;">📦 إنشاء نسخة احتياطية للعالم الآن</button>
            <div id="backups-list" style="overflow-y: auto; flex: 1;">جاري التحميل...</div>
        </div>
        <div id="settings" class="tab-content">
            <h3 style="margin-top:0; color:var(--primary);">⚙️ إعدادات السيرفر الشاملة</h3>
            <div id="config-form" class="config-grid">جاري التحميل...</div>
            <button class="btn btn-green" onclick="saveConfig()" style="width: 100%; justify-content: center; padding: 15px; font-size: 18px; margin-top: 20px;">💾 حفظ جميع الإعدادات</button>
        </div>
        <div id="crashes" class="tab-content">
            <h3 style="margin-top:0; color:var(--danger);">⚠️ محلل تقارير الكراش</h3>
            <div class="console-wrapper" style="flex: 1;"><div class="console-output" id="crash-box" style="color: #fca5a5;">جاري التحميل...</div></div>
        </div>
    </div>
    <script>
        ...
        function copyIP() { navigator.clipboard.writeText(document.getElementById('ip-display').innerText).then(() => showToast('تم نسخ الآي بي بنجاح!')); }
        let autoScroll = true;
        ...
        const cpuChart = new Chart(document.getElementById('cpuChart').getContext('2d'), { type: 'line', data: { labels: Array(20).fill(''), datasets: [{ data: Array(20).fill(0), borderColor: '#0ea5e9', backgroundColor: 'rgba(14, 165, 233, 0.1)', fill: true }] }, options: chartOptions });
        ...
        function updateCharts(cpu, ram) { cpuChart.data.datasets[0].data.push(cpu); cpuChart.data.datasets[0].data.shift(); cpuChart.update('none'); ramChart.data.datasets[0].data.push(ram); ramChart.data.datasets[0].data.shift(); ramChart.update('none'); }
        function updateStatus() {
            fetch('/api/status').then(res => { if(res.status === 401) window.location.reload(); return res.json(); }).then(data => {
                document.getElementById('cpu-text').innerText = data.cpu + '%'; document.getElementById('ram-text').innerText = data.ram + '%';
                updateCharts(data.cpu, data.ram);
                let statusEl = document.getElementById('status-text'); statusEl.innerText = data.status; statusEl.className = data.status.includes('شغال') ? 'stat-value status-online' : 'stat-value status-offline';
                document.getElementById('players-count').innerText = data.players.length;
                let ipDisplay = document.getElementById('ip-display'); ipDisplay.innerText = data.network.ip; ipDisplay.className = data.network.status === 'error' ? 'ip-badge ip-error' : 'ip-badge ip-connected';
                consoleBox.innerHTML = data.logs.join('<br>'); if (autoScroll) consoleBox.scrollTop = consoleBox.scrollHeight;
                ...
                data.players.forEach(p => { p_html += `<div class="list-item"><div class="list-item-title">👤 ${p}</div><div class="list-actions"><button class="btn btn-blue" onclick="execCmd('op ${p}')">👑 أدمن</button><button class="btn btn-red" onclick="execCmd('kick ${p}')">👢 طرد</button></div></div>`; });
                document.getElementById('players-list').innerHTML = p_html;
            }).catch(err => console.error(err));
        }
        setInterval(updateStatus, 2000);
        function sendCmd() { let cmd = document.getElementById('cmd').value; if(cmd.trim() === "") return; execCmd(cmd); document.getElementById('cmd').value = ''; autoScroll = true; }
        ...
        function sendAction(act) { if(act === 'kill' && !confirm('تحذير: الإيقاف الإجباري قد يؤدي إلى ضياع آخر التغييرات. متأكد؟')) return; fetch('/api/action', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: 'action=' + act }); }
        function loadMods() { fetch('/api/mods').then(res => res.json()).then(mods => { let html = mods.length === 0 ? '<p style="color: var(--text-muted); text-align:center; padding: 20px;">لا توجد مودات مثبتة.</p>' : ''; mods.forEach(mod => { html += `<div class="list-item"><div class="list-item-title">🧩 ${mod}</div><button class="btn btn-red" onclick="deleteMod('${mod}')">🗑️ حذف</button></div>`; }); document.getElementById('mods-list').innerHTML = html; }); }
        function uploadMod() { let fileInput = document.getElementById('mod-file'); if(fileInput.files.length === 0) return showToast('الرجاء اختيار ملف المود أولاً!', 'error'); let formData = new FormData(); formData.append("file", fileInput.files[0]); let btn = event.target; let originalText = btn.innerText; btn.innerText = "⏳ جاري الرفع..."; btn.disabled = true; fetch('/api/mods', { method: 'POST', body: formData }).then(() => { showToast('تم رفع المود بنجاح!'); fileInput.value = ''; loadMods(); btn.innerText = originalText; btn.disabled = false; }); }
        function deleteMod(modName) { if(!confirm('هل أنت متأكد من حذف المود: ' + modName + '؟')) return; fetch('/api/mods', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: 'delete=' + encodeURIComponent(modName) }).then(() => { showToast('تم الحذف بنجاح!'); loadMods(); }); }
        function loadFiles() { fetch('/api/files').then(res => res.json()).then(files => { let html = files.length === 0 ? '<p style="color: var(--text-muted); text-align:center; padding: 20px;">لا توجد ملفات.</p>' : ''; files.forEach(f => { html += `<div class="list-item"><div class="list-item-title">📄 ${f}</div><div class="list-actions"><button class="btn btn-blue" onclick="readFile('${f}')">👁️ عرض</button><button class="btn btn-red" onclick="deleteFile('${f}')">🗑️ حذف</button></div></div>`; }); document.getElementById('files-list').innerHTML = html; }); }
        function readFile(filename) { let formData = new FormData(); formData.append('action', 'read'); formData.append('target', filename); fetch('/api/files', { method: 'POST', body: formData }).then(res => res.text()).then(text => { let viewer = document.getElementById('file-viewer'); viewer.style.display = 'block'; viewer.innerText = text; viewer.scrollIntoView({behavior: "smooth"}); }); }
        function deleteFile(filename) { if(!confirm('هل أنت متأكد من حذف الملف: ' + filename + '؟')) return; let formData = new FormData(); formData.append('action', 'delete'); formData.append('target', filename); fetch('/api/files', { method: 'POST', body: formData }).then(() => { showToast('تم الحذف بنجاح!'); document.getElementById('file-viewer').style.display = 'none'; loadFiles(); }); }
        function loadBackups() { fetch('/api/backup').then(res => res.json()).then(backups => { let html = backups.length === 0 ? '<p style="color: var(--text-muted); text-align:center; padding: 20px;">لا توجد نسخ احتياطية.</p>' : ''; backups.forEach(b => { html += `<div class="list-item"><div class="list-item-title">🗄️ ${b}</div><a href="/api/backup/download/${b}" class="btn btn-green" style="text-decoration:none;">⬇️ تحميل للكمبيوتر</a></div>`; }); document.getElementById('backups-list').innerHTML = html; }); }
        function createBackup() { fetch('/api/backup', { method: 'POST' }).then(() => { showToast('بدأ إنشاء النسخة الاحتياطية...', 'info'); setTimeout(loadBackups, 5000); }); }
        function loadConfig() { fetch('/api/config').then(res => res.json()).then(data => { let html = ''; data.schema.forEach(f => { let val = data.values[f.key] !== undefined ? data.values[f.key] : f.default; html += `<div class="config-row"><span>${f.label} <small style="color:var(--text-muted); font-weight:normal;">(${f.key})</small></span>`; if(f.type === 'select') { html += `<select id="cfg-${f.key}" data-key="${f.key}">`; f.options.forEach(opt => { html += `<option value="${opt}" ${val===opt?'selected':''}>${opt}</option>`; }); html += `</select></div>`; } else { html += `<input type="${f.type}" id="cfg-${f.key}" data-key="${f.key}" value="${val}"></div>`; } }); document.getElementById('config-form').innerHTML = html; }); }
        function saveConfig() { let data = {}; document.querySelectorAll('#config-form input, #config-form select').forEach(el => { if(el.value.trim() !== '') { data[el.getAttribute('data-key')] = el.value; } }); fetch('/api/config', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) }).then(() => { showToast('تم حفظ جميع الإعدادات! أعد تشغيل السيرفر لتطبيقها.'); }); }
        function loadCrash() { fetch('/api/crash').then(res => res.text()).then(text => { document.getElementById('crash-box').innerHTML = text; }); }
    </script>
</body>
</html>
"""
